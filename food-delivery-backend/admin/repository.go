...
func (r *Repository) GetUsers(role string, isActive *bool, offset, limit int) ([]database.User, int64, error) {
...
func (r *Repository) GetVendors(isOpen *bool, offset, limit int) ([]database.Vendor, int64, error) {
	...
	err := query.Preload("User").Offset(offset).Limit(limit).Order("created_at DESC").Find(&vendors).Error
...
func (r *Repository) CountUsersByActive() (active, inactive int64, err error) {
	base := r.db.Model(&database.User{}).
		Where("role <> ?", database.RoleAdmin)

	if err := base.Where("is_active = ?", true).Count(&active).Error; err != nil {
		return 0, 0, err
	}
	if err := base.Where("is_active = ?", false).Count(&inactive).Error; err != nil {
		return 0, 0, err
	}
	return active, inactive, nil
}

func (r *Repository) CountVendorsByOpen() (open, closed int64, err error) {
	base := r.db.Model(&database.Vendor{})

	if err := base.Where("is_open = ?", true).Count(&open).Error; err != nil {
		return 0, 0, err
	}
	if err := base.Where("is_open = ?", false).Count(&closed).Error; err != nil {
		return 0, 0, err
	}
	return open, closed, nil
}

func (r *Repository) CountRidersByAvailable() (available, unavailable int64, err error) {
	base := r.db.Model(&database.Rider{})

	if err := base.Where("is_available = ?", true).Count(&available).Error; err != nil {
		return 0, 0, err
	}
	if err := base.Where("is_available = ?", false).Count(&unavailable).Error; err != nil {
		return 0, 0, err
	}
	return available, unavailable, nil
}

func (r *Repository) CountOrdersByStatus(start, end time.Time) (map[string]int64, error) {
	// Initialize all known statuses with 0 so the frontend charts remain stable.
	counts := map[string]int64{
		string(database.OrderStatusPending):   0,
		string(database.OrderStatusConfirmed): 0,
		string(database.OrderStatusPreparing): 0,
		string(database.OrderStatusReady):     0,
		string(database.OrderStatusPickedUp):  0,
		string(database.OrderStatusDelivered): 0,
		string(database.OrderStatusCancelled): 0,
		string(database.OrderStatusRejected):  0,
	}
